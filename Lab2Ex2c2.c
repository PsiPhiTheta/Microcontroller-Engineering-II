...
//2. DEFINITIONS
    #define LAB_CHAR_1 30                                                      // modify this during lab
...
void main (void) {
    ...
    unsigned int x, y, z, delay;
     ...
     while (1) {
        ...
        Delay100TCYx(125);                                                      //CHANGE IN LAB: min delay (=1/LAB_Fmax, here 0.02s or 50Hz)                                  
        for (unsigned int i = 255; i>x ; i--){ //used to avoid overflow at 255  //CHANGE IN LAB: direction (high x, low freq = "for (unsigned int i = 0 ; i<x; i++) {  ", here high x, high freq)
            Delay1KTCYx(39);                                                    //CHANGE IN LAB: unit step delay (=(1/LAB_Fmin)/255, here 4/255=0.015686s delay)
        }
