...
//4. MAIN______________________________________________________________________________________________________________________________
	void main() {
		unsigned int i = 0; //initiates only variable required
		...
		while(1){
			while(Busy1USART());
			i++;
			Write1USART(i); //send to test board
			LATF = i; //show i on LEDs
			Delay10KTCYx(250); //delay 1s
		}
	}
...
//5. FUNCTIONS_________________________________________________________________________________________________________________________
	...
	void usart_config(void) {
		Open1USART(USART_TX_INT_OFF & USART_RX_INT_OFF & USART_ASYNCH_MODE & USART_EIGHT_BIT & USART_CONT_RX & USART_BRGH_LOW,15); //baud rate 10k, interrupts off, asynchronous, 8bit
	}    
