...
void main (void) {
    ...
    unsigned int x, y, z, delay;
     ...
     while (1) {
        ...
        Delay1KTCYx(25);                                                        //CHANGE IN LAB: min delay (=LAB_Fmax/100, here 0.01s or 100Hz)                                  
        for (unsigned int i = 255; i>x ; i--){ //used to avoid overflow at 255  //CHANGE IN LAB: direction (high x, low freq = "for (unsigned int i = 0 ; i<x; i++) {  ", here high x, high freq)
            Delay100TCYx(98);                                                   //CHANGE IN LAB: unit step delay (=(1/LAB_Fmin)/255, here 1/255=0.003921s delay)
        }
